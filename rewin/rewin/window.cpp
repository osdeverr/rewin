...
namespace rewin
{
    namespace
    {
        LRESULT CALLBACK RewinWndProc(HWND hWnd,     // handle to window
    ...
    Window::Window(const WindowClass &windowClass, const lm::Vec2 &pos, const lm::Vec2 &size, const std::string &name,
                   WindowCreateMode mode, WindowFlags flags)
        : Widget{pos, size}, mWindowClass{windowClass}, mName{name}, mFlags{flags}
    {
        ...
        OnEvent<WindowResized>([this](const WindowResized &event) {
            ...
            ApplyForAll([](Widget *pWidget) { pWidget->RecalculateSize(); });
    ...
    void Window::Activate(Widget *pParent, int)
    {
        ...
        CreateWindowExA(0, pWndClassName, mName.c_str(), style, (int)pos.x, (int)pos.y, (int)size.x, (int)size.y,
                        pParent ? (HWND)pParent->GetHandle() : nullptr, nullptr, hInstance, this);
    }
